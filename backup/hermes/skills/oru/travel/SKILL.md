...
В этой фазе Oru НЕ выдаёт пассивную "поездка через N дней" — он активно ведёт Диму к поездке. Механика работает сама, без ручного триггера:
- **Ежедневно**: утренний бриф (daily-briefing) дёргает `POST /run-distant?notify=false&mark=true` и вставляет один проактивный вопрос-предложение секцией «Поездка». Темы ротируются по приоритету и срочности (билеты/виза раньше, сборы позже). Бриф — единственный потребитель дневного слота, повторно `/run-distant` руками НЕ вызывай (собьёшь ротацию).
- **Еженедельно** (пн 09:00 МСК, cron `Travel weekly research`): скрипт `travel_weekly_research.py` скребёт Tavily + Reddit/Flyertalk, дописывает датированный блок в `research-notes.md`, затем `POST /distant-weekly` отдаёт итоговую сводку (что решено / что узнал / что горит).

Если Дима спрашивает «что с поездкой» в этой фазе вручную — покажи текущий вопрос дня БЕЗ продвижения ротации:
```
curl -sS -X POST 'http://localhost:8003/run-distant?notify=false&mark=false'
```
Верни `result.question` verbatim, одной строкой-префиксом: `Поездка {trip.destination}, {days_until} дн. до старта.`

**КОГДА ДИМА СОГЛАШАЕТСЯ на предложение** (например "да, поставь alerts" / "бронируй Old Town" / "проверь визу") — СНАЧАЛА сделай работу (поиск цен через web_search/Tavily, проверка визы, сбор вариантов жилья), ПОТОМ залогируй решение, чтобы ротация перестала переспрашивать эту тему:
```
curl -sS -X POST 'http://localhost:8003/log-decision' \
  -H 'Content-Type: application/json' \
  -d '{"topic": "<flights|visa|stay|budget|poi|bookings|health|packing>", "note": "<что решено/сделано одной строкой>"}'
```

Темы ротации (id): `flights, visa, stay, budget, poi, bookings, health, packing`. Источник фактов для вопросов — `research-notes.md` в папке поездки (обновляется еженедельным кроном). Debug-состояние трекера: `GET /engagement`.
...
## Distant-phase engagement (NEW)

Эндпоинты, которые ведут Диму к далёкой поездке (фаза `scheduled_distant`). В норме вызываются автоматически (бриф + weekly cron), но знать их полезно:

### POST /run-distant — проактивный вопрос дня
```
curl -sS -X POST 'http://localhost:8003/run-distant?notify=false&mark=true'
```
Параметры: `notify` (слать в Telegram), `mark` (продвинуть ротацию — true только когда вопрос реально показывается Диме). Возврат: `result.{topic, topic_label, days_until, question}`. Бриф вызывает с `mark=true`; ручной просмотр — `mark=false`. Пропускает работу если фаза не scheduled_distant/pre_trip.

### POST /distant-weekly — недельная итоговая сводка
```
curl -sS -X POST 'http://localhost:8003/distant-weekly?notify=false'
```
Возврат: `result.summary` (что решено / что узнал за неделю / что горит / 1-2 действия). Verbatim.

### POST /log-decision — зафиксировать решение
```
curl -sS -X POST 'http://localhost:8003/log-decision' \
  -H 'Content-Type: application/json' \
  -d '{"topic": "flights", "note": "alerts на Google Flights поставлены, multi-city дороже на 15%"}'
```
Вызывай ПОСЛЕ того как сделал работу по теме — ротация перестанет переспрашивать, сводка отразит прогресс.

### GET /engagement — debug трекера
Возврат: covered-темы, last_topic, журнал decisions для активной поездки.
