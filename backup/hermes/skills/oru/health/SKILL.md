...
## Daily nudge (проактивный слой, для брифа)

Кроме недельного дайджеста health умеет один проактивный вопрос-предложение в день — он выбирается по тому, какая метрика сейчас сильнее всего просела (восстановление/сон/нагрузка/баланс зон), с anti-repeat трекером. Это часть единого кросс-доменного контракта `/daily-nudge` (тот же у travel `/run-distant`): утренний бриф (daily-briefing) опрашивает все домены и собирает секцию «Сегодня двигаем» по убыванию срочности.

В норме вызывается брифом автоматически. Ручные вызовы:
```
# посмотреть вопрос дня без продвижения трекера
curl -sS -X POST 'http://localhost:8001/daily-nudge?notify=false&mark=false'
# зафиксировать решение Димы → тема уйдёт из ротации (например он сказал "поздние созвоны — это не изменить")
curl -sS -X POST 'http://localhost:8001/log-decision?topic=sleep&note=поздние%20созвоны%20фиксированы'
# debug трекера
curl -sS 'http://localhost:8001/nudge-state'
```
Темы (id): `recovery, sleep, load, zone_balance`. Возврат `/daily-nudge`: `result.{topic, topic_label, urgency, signal, question}`. КОГДА ДИМА СОГЛАШАЕТСЯ на предложение из нуджа — сделай работу, потом вызови `/log-decision`, чтобы движок не переспрашивал.

## Доказательные исследования (PubMed)

Когда Дима спрашивает «что говорят исследования про X», «есть ли данные/доказательства что Y», «что нового по [добавка/протокол/метод]» — НЕ отвечай общей LLM-болтовнёй. Сходи в PubMed через health-контейнер:
```
curl -sS 'http://localhost:8001/research?q=<запрос на английском>&retmax=6'
```
Возврат: `result.studies[]` с полями `tier_label` (meta-analysis / systematic review / RCT / review / study), `title`, `journal`, `pubdate`, `url`. Исследования отсортированы по уровню доказательности.

Как отвечать:
- Переведи запрос Димы на английский для поиска (PubMed англоязычный).
- Дай выжимку: что показывают исследования, с явной пометкой уровня (мета-анализ весомее единичного RCT весомее обсервационного).
- Ссылки (url) на ключевые работы.
- БЕЗ «обратись к врачу» — это про данные, не про диагноз. БЕЗ эмодзи.
- Если `count: 0` — честно скажи, что качественных исследований по запросу не нашлось, не выдумывай.

Проактивный режим (крон `Health research scan`, раз в 2 недели) сам проверяет `POST /research-scan` по стоячим темам (HRV, сон, Z2, силовые/долголетие) и пишет в чат ТОЛЬКО если вышел мета-анализ/систематический обзор. Высокий порог — обычно тишина. Стоячие темы лежат в `health/src/research.py:STANDING_TOPICS`, правь там если Дима попросит следить за новой темой.
